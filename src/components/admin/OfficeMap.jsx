...
const UserPath = ({ userId }) => {
    ...
    useEffect(() => {
        ...
        const pathCollectionRef = collection(db, 'user_locations', userId, 'path');
        const q = query(pathCollectionRef, orderBy('timestamp', 'asc'));

        const unsubscribe = onSnapshot(q, (snapshot) => {
            const newPath = snapshot.docs.map(doc => {
                const data = doc.data();
    ...
    return path.length > 1 ? <Polyline positions={path} color="green" weight={5} opacity={0.8} /> : null;
};
  ...
  return (
    ...
    >
      ...
      {userLocations.map((user) => {
        ...
        {/* Live User Path */}
        {selectedUser && <UserPath userId={selectedUser.id} />}
    </MapContainer>
  );
};
